...
from emoji_frequencies import all_emojis, find_lines_with_emoji, split_list_randomly
...
def test_emoji(emoji,text):
    # print(emoji)
    lines = find_lines_with_emoji(emoji,text)
    ...
    if lines is not None and len(lines) > 10:
        ...
        #kind of looks like I need to have the text without emojis already done
        ...
        # replace this part with looping over all of X_test and see what the model would have predicted in each case
        # then compare to y_test and see what the accuracy is

        # check whether or not we have to use the transformed versions or we can just use the originals
        correct_without_emoji = 0
        correct_with_emoji = 0
        for (text,sentiment) in test:
            pmf_df = compare_pmfs(text)
            top_preds = pmf_df.idxmax()
            if top_preds["with_emoji"] == sentiment:
                correct_with_emoji += 1
            if top_preds["without_emoji"] == sentiment:
                correct_without_emoji += 1

        num_tests = len(test)
        print(correct_without_emoji/num_tests)
        print(correct_with_emoji/num_tests)
